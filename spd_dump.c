...
void print_help(void) {
	...
	DBG_LOG(
		"\n选项:\n"
		"  --wait <秒数>                  指定等待设备连接的时间\n"
		"  --stage <数字>|-r|--reconnect  尝试重新连接在 brom/fdl1/fdl2 阶段的设备\n"
		"  --verbose <等级>               设置屏幕日志的详细程度\n"
		"  --kick                         使用 boot_diag->cali_diag->dl_diag 途径连接设备\n"
		"  --kickto <模式>                使用 boot_diag->custom_diag 途径连接设备，支持的模式为0-127\n"
		"  -?|-h|--help                   显示使用帮助\n"
	);
	DBG_LOG(
		"\n运行时命令:\n"
		"\tverbose level                  设置屏幕日志的详细程度\n"
		"\ttimeout <毫秒>                 设置读写等待的超时时间（毫秒）\n"
		"\tbaudrate [波特率]              支持的波特率为57600、115200、230400、460800、921600、1000000、2000000、3250000和4000000\n"
		"\texec_addr [地址]               发送到指定的内存地址，以绕过brom对 splloader/fdl1 的签名验证\n"
		...
		"\texec                          在fdl1阶段执行已发送的文件。通常与 sml 或 fdl2 （也称为uboot/lk）一起使用\n"
		"\tpath [保存目录]\n"
