...
void print_help(void) {
	...
	DBG_LOG(
		"\n选项:\n"
		"  --wait <秒数>                  指定等待设备连接的时间\n"
		"  --stage <数字>|-r|--reconnect  尝试重新连接在brom/fdl1/fdl2阶段的设备\n"
		"  --verbose <等级>               设置屏幕日志的详细程度\n"
		"  --kick                         使用boot_diag->cali_diag->dl_diag途径连接设备\n"
		"  --kickto <模式>                使用boot_diag->custom_diag途径连接设备,支持的模式为0-127\n"
		"  -?|-h|--help                   显示使用帮助\n"
	);
	DBG_LOG(
		"\n运行时命令:\n"
		"  verbose level                  设置屏幕日志的详细程度(支持0、1或2,不影响文件日志)\n"
		"  timeout <毫秒>                 设置读写等待的超时时间\n"
		"  baudrate [波特率]              支持的波特率为57600、115200、230400、460800、921600、1000000、2000000、3250000和4000000\n"
		"  exec_addr [地址]               发送到指定的内存地址,以绕过brom对splloader/fdl1的签名验证\n"
		"  fdl [文件路径] [地址]          将文件(splloader,fdl1,fdl2,sml,trustos,teecfg)发送到指定的内存地址\n"
		"  loadexec [文件路径]            以文件名中的地址作为exec_addr,同时在执行fdl1/spl时使用该文件作为exec_file\n"
		"  loadfdl [文件路径]             以文件名中的地址作为fdl的目标地址并发送该文件到内存\n"
		"  exec                          在fdl1阶段执行已发送的文件。通常与sml或fdl2(也称为uboot/lk)一起使用\n"
		"  path [保存目录]                更改r,read_part(s),read_flash和read_mem命令的保存目录\n"
		"  nand_id [id]                  指定nand芯片的4th id参数,该参数影响read_part(s)分区大小的算法,默认值为0x15\n"
		"  rawdata {0,1,2}               rawdata协议用于加速w和write_part命令,当rawdata为1或2时,写入速度与blk_size无关\n"
		"  blk_size [字节]               设置块大小,最大为65535,加快r、w、read_part(s)和write_part(s)的速度\n"
		"  r [分区]                       读取分区(自动计算分区大小)\n"
		"  r all                         全盘备份(跳过blackbox,cache,userdata)\n"
		...
		"                                按照XML类型分区列表从设备读取分区\n"
		"                                (如果文件名以“ubi”开头,则将使用NAND ID计算大小)\n"
		"  w|write_part part_name|part_id FILE\n"
		"                                将指定的文件写入分区\n"
		"  write_parts|write_parts_a|write_parts_b save_location\n"
		"                                写入指定文件夹下所有文件到设备分区\n"
		"                                通常由read_parts得到,用write_parts_a或write_parts_b可以强制写入的槽位\n"
		"  wof part_name offset FILE     把文件写入分区偏移位置\n"
		"  wov part_name offset VALUE    把数值写入分区偏移位置(最大值为0xFFFFFFFF)\n"
		"  e|erase_part part_name|part_id\n"
		"                                擦除指定分区\n"
		"  erase_all                     擦除全部分区(谨慎使用)\n"
		"  partition_list FILE           读取emmc/ufs上的分区列表\n"
		"  repartition partition_list_xml\n"
		"                                根据XML类型分区列表重新分区\n"
		"  p|print                       打印分区列表\n"
		"  size_part|part_size part_name\n"
		"                                获取分区大小\n"
		"  check_part part_name          检测分区是否存在\n"
		"  verity {0,1}                  禁用或启用dm-verity(Android 10+)\n"
		"  set_active {a,b}              设置VAB设备上的活动槽位\n"
		"  firstmode mode_id             设置重启后设备将进入的模式\n"
	);
	DBG_LOG(
		"\n退出指令:\n"
		"  reboot-recovery               重启到recovery(仅FDL2)\n"
		"  reboot-fastboot               重启到fastboot(仅FDL2)\n"
		"  reset                         开机(FDL2和新版FDL1)\n"
		"  poweroff                      关机(FDL2和新版FDL1)\n"
	);
}
